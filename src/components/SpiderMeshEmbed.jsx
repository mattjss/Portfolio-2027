...
export default function SpiderMeshEmbed({ muted = true }) {
  const ref = useRef(null);

  // unmute on hover / mute on leave — the page listens for { type:"setMuted" }
  useEffect(() => {
    const f = ref.current;
    if (!f) return;
  ...
  return (
    <iframe
      ref={ref}
      src="/playground/spider-mesh/index.html?embed=1"
      title="Spider Mesh"
      scrolling="no"
      tabIndex={-1}
      allow="autoplay"
